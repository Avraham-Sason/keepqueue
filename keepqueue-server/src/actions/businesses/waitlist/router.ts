...
import { authGuard, requireRecordAccess, requireSelfOrBusinessOwner, validateBody } from "../../../middlewares";
import { addToWaitlistSchema, deleteFromWaitlistSchema } from "./schemes";

const waitlistRouter: Router = express.Router();

waitlistRouter.post("/add", authGuard(), validateBody(addToWaitlistSchema), requireSelfOrBusinessOwner(), SAddToWaitlist);
